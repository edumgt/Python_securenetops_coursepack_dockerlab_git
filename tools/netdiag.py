...
def main():
    ...
    ap.add_argument("--targets", default="configs/targets.yml")
    ap.add_argument("--report", default="reports/netdiag.json")
    args = ap.parse_args()
    ...
    summary = {"total": len(items), "ok": sum(1 for x in items if x.get("ok")), "fail": sum(1 for x in items if not x.get("ok"))}
    rep = Report("netdiag", utcnow_iso(), ok, summary, items)
